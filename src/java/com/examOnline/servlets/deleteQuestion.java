...
public class deleteQuestion extends HttpServlet {
      ...
      protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            try ( PrintWriter out = response.getWriter()) {
                  ...
                  Integer dQid = Integer.parseInt(request.getParameter("dQid"));
                  out.println("<h1>Servlet deleteQuestion at " + dQid + "</h1>");
                  questionDao dao = new questionDao(ConnectionProvider.getConnection());
                  boolean f = dao.deleteQuestion(dQid);
                  HttpSession s = request.getSession();
                  if (f) {
                        out.println("done");
                        Message msg = new Message("Question Delete Successfully...", "success ", " bg-green-100 rounded-lg py-5 px-6 mb-4 text-base text-green-700 mb-3");
